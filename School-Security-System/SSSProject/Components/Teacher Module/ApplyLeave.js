...
const ApplyLeave = () => {
  ...
  const getUID = async () => {
    try {
      ...
      if (sessionString) {
        ...
        const { CardID ,uid } = session;
        setUserUid(uid);
        getName(CardID);
         // Pass the UID to getClass
      }
    } catch (error) {
  ...
  function getName(uid) {
    ...
    onValue(classRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setisClassTeacher(snapshot.val().classTeacher);
  ...
  const fetchTeacherLeaveData = async () => {
    ...
    console.log('Fetching teacher leave data...');
    try {
      const dbRef = ref(getDatabase());
      const teacherLeaveRef = child(dbRef, 'LeaveRequests');
      ...
      if (dataSnapshot.exists()) {
        ...
        for (const date in data) {
          ...
          // Iterate through each leave request within the date
          for (const id in dateData) {
            const leave = dateData[id];
            formattedLeaveData.push({
              ID: leave.ID,
              Name: leave.Name,
              reason: leave.reason,
              applyDate: leave.applyDate,
              singleDay: leave.singleDay,
  ...
  const handleApplyLeave = () => {
    ...
    const applyDate = `${currentDate.getDate().toString().padStart(2, '0')}-${(currentDate.getMonth() + 1)
      ...
      if (isSingleDay) {
        leaveRequest.singleDay = selectedSingleDate;
      } else {
        leaveRequest.startDate = selectedStartDate;
        leaveRequest.endDate = selectedEndDate;
      }
  
      const dbRef = ref(getDatabase(), 'LeaveRequests/' + applyDate + '/' + userUid);
      set(dbRef, leaveRequest)
        .then(() => {
  ...
  const filterUserLeaves = () => {
    // Filter the teacherLeaveData array based on the userUid
    const userLeaves = teacherLeaveData.filter((leave) => leave.ID === userUid);
...
const styles = StyleSheet.create({
  ...
  SelectingLabel: {
    ...
    color: 'white',
    textAlign: 'center',
    backgroundColor: '#054f96',//dark blue color
    borderRadius: 10,
...
inputField: {
  ...
  label:
  {
    ...
    color: 'black',
    textAlign: 'center',
    backgroundColor: 'lightgrey',//dark blue color
    borderRadius:80,
    borderBottomLeftRadius: 0,
    borderBottomRightRadius: 0,
  },
